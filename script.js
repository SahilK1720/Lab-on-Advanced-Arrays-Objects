...
const result = strings.filter(function(strings){
...
const numSq = numbers.map(function(numbers){
...
const sent = alphabets.reduce(function(final, current){
...
const firstEvenNumbers = number.find(function(number){
  return number%2 === 0;
})
...
const indexFind = ranks.findIndex(function(rank){
  return rank === 7;
})
...
//3:8
const num = [2, 4, 6, 8];
const output = num.forEach(function(currentValue, index){
...
const mark = marks.some(function(mark){
  return mark < 45;
})
console.log(mark)
...
const allPositive = positive.every(function(element){
  return element > 0;
})
...
//2. Using the hasOwnProperty check whether the person object has the property "bike", "house".
let res = person.vehicles.hasOwnProperty('bike');
console.log(res);
